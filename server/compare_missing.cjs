...
async function compareExact() {
  console.log('🔄 Identifying the 2 missing transactions...');
  ...
  unverified.forEach(tx => {
    ...
    unverifiedCount[key] = (unverifiedCount[key] || 0) + 1;
  });
  ...
  fileRows.forEach((row, i) => {
    ...
    if (fileOccurrence[key] > (unverifiedCount[key] || 0)) {
      missing.push({ line: i + 2, date: dateStr, time, vendor, amount });
    }
  });

  console.log('\n--- Missing Transactions (Not in New tab) ---');
  missing.forEach(m => {
