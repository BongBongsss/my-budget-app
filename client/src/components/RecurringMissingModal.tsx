...
const RecurringMissingModal = ({ items, onClose, onAdded }: Props) => {
  ...
  return <div className="entry-modal-overlay recurring-missing-overlay">
    <section className="entry-modal recurring-missing-modal" role="dialog" aria-modal="true" aria-label="미반영 고정비 항목">
      <div className="entry-modal-header"><div><h3>미반영 고정비 항목</h3><p>승인된 거래와 비교해, 예정일이 지난 항목입니다.</p></div><button className="btn-icon" onClick={onClose}><X size={20} /></button></div>
